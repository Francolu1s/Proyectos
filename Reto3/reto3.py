...
def CambiarClave(claveactual):
    if ValidacionDatos(input("Por favor ingrese su contraseña actual: "), claveactual):
        ...
        if ValidacionDatos(nuevaclave,claveactual):
            ErrorConMensaje ("Error") #("La nueva contraseña no puede ser igual a la anterior.")
            return claveactual
        else:
    ...
    for x in range (0,3):
        ...
        if lat >= 6.077 and lat <= 6.284:
            ...
            if lon >= -76.049 and lon <= -75.841:
                ...
                ErrorConMensaje("Error coordenada")
                listaduplicada=[]
                exit()#return listaduplicada
        else:
            ErrorConMensaje("Error coordenada")
            listaduplicada=[]
            exit() #return listaduplicada
...
def ImprimirCoordenads(listaoriginal):
    ...
    if choice !=1 and choice !=2 and choice !=3:
        ...
        exit() #return #En caso de error hacemos un return
    else:
