...
#Si el nombre de usuario es Esfuerzo2021 se mostrará el mensaje "Logré llegar hasta aquí!!!" y salir

#si en el menu se ingresa la opcion 2021 mostraremos un mensaje especial:
#dime en que latitud estás y te diré si estás por encima o debajo del ecuador

#si el nombre de usuario y la contraseña es PR0GR4M4D0R
#se le preguntará al usuario un número de datos que desea ingresar.
#se preguntará la cantidad de veces ingresada por un un número
#se calculará el promedio de los datos ingresados.

#si el usuario ingresa 2022 en el menu se le pedirá una longitud
#se validará si dicha coordenada está dentro de america del sur
#de ser así se mostrarán 3 mensajes diferentes basados en estos rangos
#Si la longitud ingresada está entre  67.401 y  81.296 usted ha entrado al primer rango
#Si la longitud ingresada está entre 54.316 y 67.402 usted ha entrado al segundo rango
#Si la longitud ingresada está entre  35.833 y 4.316 usted ha entrado al tercer rango

#region VARIABLES GLOBALES
#creamos una de las variables de los datos que debemos validar
easteregg1="Esfuerzo2021"
...
def ImprimirRestaurantesFav(listaoriginal):
    ...
    if opcion == 1 or opcion ==2 or opcion ==3:
        ...
        informacion["actual"]=restauranteactual
...
def LeerArchivo(archivo):
    try:
        ...
        ErrorConMensaje("Error en creacion de fichero")
        return [[10.127,-74.950,7],[10.122,-74.908,12],[10.305,-75.040,32],[10.127,-74.950,5000]]
        
    except FileNotFoundError:
        ErrorConMensaje("Error el archivo no existe")
        return [[10.127,-74.950,7],[10.122,-74.908,12],[10.305,-75.040,32],[10.127,-74.950,5000]]
    except ValueError:
...
def CrearArchivo():
    if alistamiento:
        try:
            ...
            print("Exportando...")
            time.sleep(2)
            export=int((input("Presione 0 para salir o 1 para guardar: ")))
            if export == 0 or export == 1:
                if export == 0:
                    return
                else:
                    exit()
...
#función para el easter egg 2
def EasterEgg2():
    os.system("cls")
    #basado en condicionales simples averiguamos si la latitud ingresada está por debajo
    #o por encima del ecuador.
    easter2=float(input("Dime en que latitud estás y te diré si estás por encima o debajo del Ecuador:"))
    if easter2 > 0:
        print("Estás por encima del Ecuador")
    elif easter2 < 0:
        print("Estás por debajo del Ecuador")
    else:
        print("Estás justamente en el Ecuador.")
    time.sleep(2)
    exit()

#función para el easter egg 3
def EasterEgg3():
    #creamos variables para un contador, una lista vacía y una suma
    contador=0
    numeros=[]
    suma=0
    #pedimos la cantidad de datos que el usuario desea ingresar
    cantidad=int(input("Ingrese la cantidad de datos a la cual desea desea calcular el promedio: "))
    #mientras contador sea menor a cantidad repetiremos el ciclo
    while contador<cantidad:
        #pedimos el valor
        valor=float(input(f"Ingrese el valor {contador+1}: "))
        #lo agregamos a una lista temporal
        numeros.append(valor)
        #actualizamos el total de la suma con el valor ingresado y aumentamos el contador.
        suma+=valor
        contador+=1
    #mostramos el promedio de los datos ingresados luego de terminar el ciclo
    print(f"El promedio es: {(suma/len(numeros))}")

#easter egg 4 basado en condicionales simples mostramos mensajes diferentes
#según el dato ingresado por el usuario.    
def EasterEgg4():
    ee4=float(input("Por favor ingrese una longitud: "))
    if ee4 > -81.296 and ee4 <  -67.401:
        print("El huso horario es -5")
        exit()
    elif ee4 > 54.316 and ee4 <  67.402:
        print("Usted ha entrado al segundo rango")
        exit()
    elif ee4 > 35.833 and ee4 <  45.316:
        print("Usted ha entrado al tercero rango")
        exit()
    else:
        ErrorConMensaje("Rango inválido")
        exit()
        

print("Bienvenido al sistema de ubicación para zonas públicas WIFI")
UsuarioIngresado=input("Ingrese su usuario: ")
#validamos el usuario y contraseña para llamar a un easter egg

#si no se aplica el easter egg continuamos el programa como siempre.
if ValidacionDatos(UsuarioGuardado,UsuarioIngresado):
    clave1 = input("Ingrese su contraseña: ")
    if ValidacionDatos(clave1, "m1s10nt1c"):
        EasterEgg3()
     
    elif ValidacionDatos(clave1, ClaveGuardada): 
            ...
            while contadorerrores<5:
                ...
                if opcionelegida > 0 and opcionelegida < 8:
                    ...
                    if opcionelegidalista==opc1:
                        ...
                        exit()
                #validamos otros dos easter eggs dentro de menu
                elif ValidacionDatos(opcionelegida,2021):
                    EasterEgg2()
                elif ValidacionDatos(opcionelegida,2022):
                    EasterEgg4()         
...
#realizamos la última validacion que nos lleva al último easter egg    
elif ValidacionDatos(UsuarioIngresado,easteregg1):
    print("Logré llegar hasta aquí!!!")
    exit()
